...
function createTableInscrits() {
    ...
    tabDives.forEach(function (item) {
        if(item.get_id_planned_dive() == idPlannedDive) {
            idDive = item.get_id();
...
function setButtonListener(){
    ...
    attributionAutomatique.addEventListener('click', event => {
        let good = funcAttributionAutomatique(); // Run myFunction with a timeout of 5 seconds (5000 milliseconds)
        //validerPalanquées(attributionAutomatique);
        if(good == false){
            //console.log("Erreur dans l'attribution automatique");
        }
        else{
...
function validerPalanquées(attributionAutomatique){
    ...
    if(isDiveTeamExist){
        ...
        setTimeout(function(){
            ...
            SocketManager.updateInfosForAllUsers(idPlannedDive);
...
function funcAttributionAutomatique(){
    ...
    if(nbDiversPlongeeActuelle < 2){
        alert("Il n'y a pas assez de plongeurs pour créer une palanquée");
        return false;
    }
...
function updatePage(id, path){
    ...
    if(id == idPlannedDive && path == window.location.pathname){
